...
async function processContentHandler(req, res, getMongoCollection) {
  try {
    ...
    if (req.query.pdf) {
      ...
      textForEmbedding = await new Promise((resolve, reject) => {
        ...
        parser.on('pdfParser_dataReady', pdfData => {
          ...
          if (!rawText.trim()) {
            console.warn(`[processContentHandler] WARNING: pdf2json extracted text is empty for PDF: ${pdfPath}. Trying pdftotext fallback.`);
            try {
              ...
              console.error('[processContentHandler] pdftotext fallback extraction failed:', err.message);
            }
          } else {
      ...
      // Also try extracting text with pdftotext for comparison
      try {
        const { execSync } = require('child_process');
        ...
        // Optionally, you could use pdftotextOutput as fallback if pdf2json fails
      } catch (err) {
        console.error('[processContentHandler] pdftotext extraction failed:', err.message);
      }
    } else {
    ...
    console.log('[processContentHandler] Chunk contents:');
...
// Superseded by src/routes/handlers/processContentHandler.ts
module.exports = require('../../src/routes/handlers/processContentHandler');
